...
Thanks for thinking about contributing. This document covers the things you'll
...
- Keep `app.py` route handlers thin — push real work into `ai/`, `core/`,
  `scanners/`.
- New files belong in the matching package: AI work → `ai/`, generic
  infrastructure → `core/`, scanners → `scanners/`. Don't drop new
  modules at the repository root.
- English in user-facing strings (UI labels, REST responses, log lines).
  Comments can be any language but English preferred for shared code paths.

### Frontend (TypeScript / React)

- Avoid runtime style libraries; inline styles + CSS variables match the
  existing pattern.
...
3. Tag: `git tag -a vX.Y.Z -m "..."` → `git push --tags`.
