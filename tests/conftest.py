...
# Make the repo root importable so `from ai.utils import ...` works.
...
def _stub_env(tmp_path_factory):
    """Default env that lets app.py import without touching a real license
    server or DB. Anything that needs the real stack must mark itself
    ...
    os.environ.setdefault("LICENSE_KEY", "test-shared-secret-32chars-_______")
